...
- [ ] 숙박 추가/수정 시 체크인~체크아웃 날짜 범위로 일정에 숙소 항목 자동 생성
- [ ] 숙박 삭제 시 연동된 일정 항목 자동 삭제
- [ ] 항공편 폼에 사진 업로드 → LLM 비전으로 정보 자동 추출
- [ ] 렌트카 폼에 사진 업로드 → LLM 비전으로 정보 자동 추출
- [ ] 숙박 폼에 사진 업로드 → LLM 비전으로 정보 자동 추출
- [ ] 여행 공유 초대 링크 생성 (trip_shares 테이블)
- [ ] 초대 링크로 접속 시 공유 멤버로 등록
- [ ] 공유 멤버 목록 표시 및 관리 (오너만 삭제 가능)
- [ ] 공유된 여행에서 모든 멤버가 실시간 편집 가능
- [ ] 실시간 동기화 (polling 방식 - 10초 간격 자동 갱신)
