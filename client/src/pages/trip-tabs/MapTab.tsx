          ...
          {tripDays.map((day, idx) => {
            ...
            return (
              ...
              >
                ...
                <span className={`font-bold leading-none transition-all duration-200 ${compactDateSelector ? "text-sm" : "text-lg"}`}>{format(day, compactDateSelector ? "M.d" : "d")}</span>
                <span className={`text-xs opacity-70 transition-all duration-200 ${compactDateSelector ? "max-h-0 opacity-0 -mt-1" : "max-h-4 opacity-70"}`}>{format(day, "M.d")}</span>
                <span className={`text-xs mt-0.5 transition-all duration-200 ${isSelected ? "text-primary-foreground/70" : "text-muted-foreground"} ${compactDateSelector ? "max-h-0 opacity-0 -mt-1" : "max-h-4 opacity-100"}`}>Day {idx + 1}</span>
              </button>
            );
          })}
        </div>
