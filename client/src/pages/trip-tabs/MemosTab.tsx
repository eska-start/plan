...
function renderContentWithLinks(content: string) {
  return content.split(URL_REGEX).map((part, idx) => {
    if (part.match(URL_REGEX)) {
      return <a key={`${part}-${idx}`} href={part} target="_blank" rel="noopener noreferrer" className="text-primary underline underline-offset-2 break-all">{part}</a>;
    }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {memos?.map(m => (
            ...
            >
              ...
              {m.content && (
                <p className="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap mb-3 line-clamp-6">{renderContentWithLinks(m.content)}</p>
              )}
